...
def init_exchange(proxy=None):
    ...
    if proxy:
        exchange_config['httpProxy'] = proxy
...
def fetch_trades_symbol(exchange, symbol, since_ms, until_ms):
    return exchange.fetch_my_trades(symbol=symbol, since=since_ms, params={'until': until_ms})
...
def fetch_trades_all(_exchange, symbols, since_date):
    ...
    # Create 7-day ranges for fetching
    time_ranges = []
    ...
    while current_start.timestamp() * 1000 < until_timestamp:
        ...
        time_ranges.append((current_start_ms, next_end_ms))
        current_start = next_end

    # Fetch trades concurrently per symbol & time range
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for symbol in symbols:
            for start_ms, end_ms in time_ranges:
                futures.append(executor.submit(fetch_trades_symbol, _exchange, symbol, start_ms, end_ms))
...
def main():
    ...
    st.markdown("""
        ...
        <style>
            ...
            @media (prefers-color-scheme: dark) { .stApp { background-color: #1f2937; } }
            .sidebar .sidebar-content { background-color: #ffffff; border-right: 1px solid #e5e7eb; }
            @media (prefers-color-scheme: dark) { .sidebar .sidebar-content { background-color: #374151; border-right: 1px solid #4b5563; } }
            h1, h2, h3, p { color: #1f2937; }
            @media (prefers-color-scheme: dark) { h1, h2, h3, p { color: #f3f4f6; } }
            .stMetric {
    ...
    with st.sidebar:
        ...
        refresh = st.button("🔄 Refresh Data", help="Fetch latest data from Binance.")
    ...
    # Read proxy URL from environment variable PROXY_URL
    proxy = os.environ.get('PROXY_URL')
    exchange = init_exchange(proxy=proxy if proxy else None)
